  ...
  const renderActiveShape = (props: any) => {
    ...
    return (
      <g>
        ...
        <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#333" fontSize={12} fontWeight="500">{payload.name}</text>
        <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} dy={18} textAnchor={textAnchor} fill="#999" fontSize={12}>{`$${value.toFixed(2)} (${(percent * 100).toFixed(1)}%)`}</text>
      </g>
    );
  };
